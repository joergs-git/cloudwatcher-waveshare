...
// Forecast window: 15-min resolution, up to 97 points covering -12h to +12h
#define MB_FORECAST_HOURS 97
...
typedef struct {
    ...
    time_t timestamps[MB_FORECAST_HOURS];       // Unix timestamp per 15-min slot
...
// Fetch 15-min cloud cover forecast (blocking, uses PSRAM buffer)
// Populates data with 24h of 15-min cloud cover centered on now (-12h to +12h)
